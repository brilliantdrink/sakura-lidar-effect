  ...
  return (
    <>
      ...
      <SwitchPrimitive.Control
        class={cn(
          "inline-flex h-5 w-8 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent bg-input transition-[color,background-color,box-shadow] data-[disabled]:cursor-not-allowed data-[checked]:bg-primary data-[disabled]:opacity-50",
          local.class
        )}
        {...others}
      >
...
const SwitchThumb = <T extends ValidComponent = "div">(
  ...
  return (
    <SwitchPrimitive.Thumb
      class={cn(
        "pointer-events-none block size-4 translate-x-0 rounded-full bg-background shadow-lg ring-0 transition-transform data-[checked]:translate-x-3",
        local.class
      )}
      {...others}
    />
